...
import { sparklinePath, useCountUp, useMounted } from "./anim.js";
import {
  CATEGORY_TIP,
  CONFIDENCE_TIP,
  FRICTION_TIP,
...
export function BrandMark({ size = 30 }: { size?: number }): ReactElement {
  return (
    <span className="brandmark" style={{ width: size, height: size }}>
      ...
      <span className="sr-only">Harness Health</span>
    </span>
  );
}
...
/**
 * The hero score ring: three health arcs that sweep in on mount, with the
 * composite health score counting up in the center.
 */
  ...
  return (
    <div className="scorering">
      <svg
        ...
        viewBox={`0 0 ${size} ${size}`}
        role="img"
        aria-label={`Harness health score ${score} of 100`}
      >
        {rings.map((ring, i) => {
          const radius = center - stroke / 2 - i * (stroke + gap);
      ...
      <div className="scorering-center">
        ...
        <div className="scorering-label">Health score</div>
      </div>
    </div>
  );
}
...
export function alignmentSplit(report: HealthReport): {
  human: number;
  agent: number;
  band: AlignmentBand;
} {
  ...
  const paths: Record<Tab, ReactNode> = {
    ...
    browse: (
      <>
        ...
        <circle cx="5" cy="15" r="1.4" fill="currentColor" stroke="none" />
      </>
    ),
    review: (
      <>
...
const PHASE_LABEL: Record<HealthReviewPhase, string> = {
  idle: "Idle",
  running: "Running",
  ready: "Ready",
};
...
/**
 * The always-present Private Device Sync call-to-action, pinned to the bottom-left of
...
export function CloudSyncCTA({
  ...
  return (
    <button type="button" className="cloud-cta" onClick={onClick}>
      ...
      <span className="cloud-cta-title">Set up Private Sync</span>
...
export function Sidebar({
  ...
  const tabGroups: { title: string; items: { tab: Tab; label: string }[] }[] = [
    {
      title: "Dashboard",
      items: [
        { tab: "today", label: "Summary" },
        { tab: "browse", label: "Browse" },
        { tab: "review", label: "Trends" },
        { tab: "lab", label: "Habits" },
      ],
    },
    {
      title: "Care",
      items: [
        { tab: "config", label: "Config Health" },
        { tab: "chat", label: "Coach" },
      ],
    },
    {
      title: "App",
  ...
  const lastLabel = lastReviewAt
    ? new Date(lastReviewAt).toLocaleTimeString([], {
        hour: "numeric",
        minute: "2-digit",
      })
    : "—";
    ...
    <aside className="sidebar">
      <div className="side-brand">
        ...
        <span className="side-brand-text">
          <span className="side-brand-name">Harness Health</span>
          <span className="side-brand-sub">Vitals · Habits · Reviews</span>
        </span>
      </div>
      <nav className="side-nav" aria-label="Harness Health sections">
        {tabGroups.map((group) => (
          <Fragment key={group.title}>
            <span className="side-nav-heading">{group.title}</span>
            {group.items.map(({ tab, label }) => (
              <NavItem
                ...
                active={active === tab}
                badge={
                  tab === "review" && unreviewed > 0 ? unreviewed : undefined
                }
      ...
      <section className="side-library" aria-label="Health area overview">
        <span className="side-nav-heading">Browse Areas</span>
        <div className="side-area-row efficiency">
          <span />
          <b>Efficiency</b>
        </div>
        <div className="side-area-row effectiveness">
          <span />
          <b>Effectiveness</b>
        </div>
        <div className="side-area-row alignment">
          <span />
          <b>Alignment</b>
          <em className="tnum">{alignment}</em>
        </div>
      </section>
    ...
    <article className={`finding${compact ? " compact" : ""}`}>
      ...
      {!compact ? (
        <p className="finding-watch">
          Next Health Review watches: {finding.reflection}
        </p>
      ) : null}
 ...
 * routes the whole card to the Health Review on click.
 */
  ...
  return (
    <div className="review-progress">
      <div className="review-progress-head">
        <span className="review-stage-now">
          {paused ? "Paused" : (stage ?? "Preparing Health Review")}
        </span>
        <span className="review-pct tnum">{pct}%</span>
      </div>
      <div className="review-bar">
        <div
          className={`review-bar-fill${paused ? " paused" : ""}`}
          style={{ width: `${pct}%` }}
        />
      </div>
          ...
          const cls =
            progress >= st.at + 0.0001 && stage !== st.label
              ...
              : stage === st.label
                ...
                : "pending";
          return (
            <div key={st.label} className={`review-stage ${cls}`}>
              <span className="review-stage-dot" />
              <span>{st.label}</span>
            </div>
          );
        })}
...
export function AreaChart({
  ...
  return (
    <svg
      ...
      preserveAspectRatio="none"
      role="img"
      aria-label="Health Review trend"
    >
      <defs>
      ...
      {pts.map(([x, y], i) => (
        <g
          ...
          aria-label={`${labels[i] ?? "Health Review"} · ${values[i]}`}
          onClick={() => onPick?.(i)}
          onKeyDown={(e) => {
...
/** Grouped vertical bars — one cluster per review, one bar per metric. */
      ...
      {primary?.values.map((v, i) => {
        ...
        return (
          <g
            ...
            role="button"
            tabIndex={onPick ? 0 : undefined}
            aria-label={`${labels[i] ?? "Health Review"} · ${v}`}
            onClick={() => onPick?.(i)}
            onKeyDown={(e) => {
              if (e.key === "Enter" || e.key === " ") onPick?.(i);
            }}
  ...
  if (!previous) {
    return (
      <div className="compare-empty">
        First Health Review — no comparison yet.
      </div>
    );
  }
  const ringScore = (r: HealthReport, key: string): number =>
    r.rings.find((x) => x.key === key)?.score ?? 0;
  const reask = (r: HealthReport): number => {
