...
Each "review" term has a plain-language meaning so the metaphor never obscures the
mechanics.
...
| **Harness Health** | the product | The macOS menu-bar health/reflection app for harnesses. |
| **Idle** | idle/overnight trigger | The condition (idle, scheduled, or manual) under which a review runs. The harness isn't actively coding. |
| **Review / Health Review** | analysis run | One background analysis run over a window of activity. Produces a Health Report. |
| **Health Report** | the output | The Apple-Health-style artifact: vitals, trends, findings, experiments, memory proposals, digest. |
| **Deterministic Vitals** | deterministic pass | The no-LLM stage: compute vitals, trends, dedupe, organize. Fast, reliable, offline. |
| **Insight** | LLM pass | The model-driven stage: read transcripts, find patterns, draft findings/experiments/config recs. |
...
| **Finding** | an insight | A grounded Insight insight: Win, Mistake, Risk, or Opportunity, with evidence + confidence + a proposed action. |
...
| **Experiment** | a tracked test | A hypothesis-driven, measurable change (e.g. "medium effort for UI") that the next review grades. |
...
| **Grading** | scoring an experiment | The next review's measured verdict on a running experiment: Improved / Inconclusive / Regressed. |
...
| **Health Review Engine** | the analyzer | The component running Deterministic Vitals + Insight + assembly. See `15`. |
| **Redaction** | secret scrubbing | The layer that scrubs secrets/PII from any excerpt before a cloud LLM call. |
| **Local-only mode** | no-cloud mode | A configuration where no data leaves the device; Insight is local-model or disabled. |
...
- "Review" = the run; "Health Report" = the artifact; "Reflection" = the human
