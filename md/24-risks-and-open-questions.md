...
|---|---|---|
| **Findings aren't good enough** | If REM produces generic or wrong insights, nobody opens the report twice | Phase 0 spike against real data; strict evidence grounding + confidence filtering (`15`); cap to a few high-confidence items |
...
| **Cost of overnight dreams** | LLM spend could be uncomfortable | Per-dream budget guard; cheap Deep Sleep does the heavy lifting; tier models (`15`) |
...
| **Prompt injection via transcripts** | Transcript content could try to steer REM | Treat content as data, not instructions; structured outputs; no execution from REM (`20`) |

## Open questions (need a decision)

1. **Dream window for irregular schedules.** How to define "yesterday" for
...
3. **Local-model REM viability.** Which local models are good enough for findings,
...
7. **Idle/sleep trigger semantics on macOS.** Reliable detection of "harness
