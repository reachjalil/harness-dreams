# 15 · Health Review Engine

*Status: 🟡 Draft*

The Health Review Engine is the analysis pipeline that turns a window of normalized
events into a Health Report. It runs two passes — **Deterministic Vitals** (deterministic)
and **Insight** (LLM) — and is the most technically central component.
...
## Stage 1 — Deterministic Vitals (deterministic)
...
Output: a complete, grounded facts package. If Insight fails, the report still ships
with vitals + trends.

## Stage 2 — Insight (LLM-driven)
...
Insight sub-tasks (each a focused, schema-constrained call):
...
- Insight only sees **facts + cited excerpts**; it must reference event IDs that
...
- Build the `HealthReport` artifact and persist (`12`).
...
guesses (see `13`/`24`). A **token budget per review** is enforced; overnight
reviews get a larger budget than on-demand ones. Local-model execution (e.g.
Ollama) is a future privacy option (`20`).

## Scheduling & execution

- A background **scheduler** (`17`, `18`) triggers reviews (scheduled/idle/manual).
- Reviews run as a **resumable job** with per-stage checkpoints; interruption
  restarts from the last completed stage.
- **Concurrency**: one review at a time; new triggers queue or coalesce.
- **Budget guard**: hard cap on token spend; degrade gracefully (skip the
  expensive cross-project pass before failing the whole review).

## On-demand fast path

For "Run Health Review," return value progressively:
1. Deterministic Vitals vitals render immediately (deterministic, ~instant).
2. Insight findings stream in as each schema-constrained call completes.
3. The report is "ready" once core findings land; cross-project can finish
   lazily.

## Determinism & testability

- Deterministic Vitals is fully unit-testable against fixture transcripts.
- Insight is tested with **recorded fixtures** + schema validation + golden-ish
